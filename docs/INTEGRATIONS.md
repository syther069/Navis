...
| Agent link              | Implemented, provider-configured | Owner action `POST /api/agents/{slug}/clawpump-link` on a saved persistent agent: create via `POST /agents` or attach an id the key lists under `GET /agents` whose registered `walletAddress` equals the signed-in wallet (the Partner key is shared, so the wallet signature is the entitlement proof; other users' identities are never listed). Atlas and public demo rows refused; one link per agent, one Navis agent per external id (database unique index `agents_external_agent_id_unique`). `GET` refreshes identity and token address via `GET /agents/{id}`. |
