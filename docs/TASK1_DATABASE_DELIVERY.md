...
| Create Agent form | Generates one request key per reviewed mandate, blocks double submission, and shows distinct "not saved" states for storage, sign-in and mandate failures. A retry after a storage failure repeats the same request and cannot create a second agent; the message says the save is unconfirmed rather than claiming nothing was stored. |
...
While this task was in progress the owner enabled the public database, session secret and app origin on Vercel (tracked as a separate task). Checked directly on 2026-09-21: the production Neon database holds journal entries `0000` through `0008` with hashes identical to the repository files, including `agents.client_request_id` and its unique index, so the code in this delivery deploys against a matching schema. `GET https://navis-gilt.vercel.app/api/health` after this commit deploys should report `database.status = ok` (the check now also requires the `0008` column and index), `authenticationOrigin` and `walletSessions` `configured`. The production check is then: create an agent through `/agents/new` with a signed-in wallet, reload `/agents` to see it persisted, and confirm a second submit of the same reviewed mandate does not create a twin.
