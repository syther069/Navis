...
| F14 P2                      | No shared quotas on several costly/record-creating endpoints             | Fixed in final hardening: database-shared verified-wallet quotas and public RPC quotas, bounded burst protection, no-store 429/Retry-After, fail-closed 503 |
...
| F18 P2                      | Smoke report only saved on success                                       | Fixed in final hardening: save partial checks, failed step and explicit failed status in cleanup; retain nonzero exit and replace stale success             |
...
F14 and F18 were completed after the original audit validation below. Sources, scope and deployment boundaries are in [`docs/FINAL_RELEASE_NOTES.md`](docs/FINAL_RELEASE_NOTES.md). Focused validation passed 103 quota/auth/route tests and two smoke failure-report tests. The final delivery records the new combined validation and exact PR head; the original 511-test result below is historical, not a claim that unchanged checks cover the additions.

F14 files: `lib/auth/operation-quota.ts`, authenticated agent/decision/ClawPump/Meteora prepare/simulate/confirm routes, and public Solana slot/Meteora pool lookup routes. New tests: `operation-quota.test.ts`, `operation-quota-routes.test.ts`; existing route tests retain their functional assertions.

F18 files: `scripts/smoke-judge-flow.mjs`, `tests/smoke-failure-report.test.ts`. No schema, dependency, execution flag or broadcast-gate changes.
