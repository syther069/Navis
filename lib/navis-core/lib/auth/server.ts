...
import { authChallenges, authSessionRevocations, users } from "@workspace/navis-core/lib/db/schema";
...
async function isSessionRevoked(jti: string) {
  ...
  try {
    ...
    logger.error({ detail: error instanceof Error ? error.name : typeof error, }, "[navis:auth.session] revocation check failed:");
...
export async function revokeAuthenticationSession(
  ...
  try {
    ...
    logger.error({ detail: error instanceof Error ? error.name : typeof error, }, "[navis:auth.session] revocation failed:");
    return "unavailable";
  }
  ...
  try {
    ...
    logger.error({ detail: error instanceof Error ? error.name : typeof error, }, "[navis:auth.session] revocation pruning failed:");
  }
