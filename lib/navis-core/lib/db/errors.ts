...
const PUBLIC_MESSAGE: Readonly<Record<DatabaseErrorCode, string>> = Object.freeze({
  ...
  database_schema_mismatch:
    "Persistent storage schema is unavailable or incompatible with this NAVIS release. Configure the existing NAVIS database; no migrations have been run.",
...
export function logDatabaseError(scope: string, error: DatabaseError) {
  ...
  logger.error({ scope, code: error.code, sqlstate, constraint }, "Database operation failed");
}
