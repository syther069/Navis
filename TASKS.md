...
**Final sync, 2026-09-20 (late evening IST):** Since the restoration update the following shipped and are documented in `README.md`, `docs/EVIDENCE.md` and `docs/SUBMISSION_READINESS.md`: fresh decision runs (`POST /api/decisions/run`) with Balanced and Oversized scenarios and browser-verified receipts; the `/agents` list and persisted one-transaction runs for owner-created demo-mode agents (devnet and mainnet agents refused with 409); the live PreStocks catalogue as the Atlas asset universe with a research view and proposal-consistent allocation; the landing screen at `/`; assurance badges on every receipt, decision, run result and ledger row; real HTTP 404 for unknown decision and proof ids; Meteora intent binding, persist-before-broadcast and the stock-paired `navis-stock-exposure-v1` quote profile (broadcast hard-blocked); the MIT licence; and the GitHub CI pipeline. The full gate now has nine steps and passes with 46 test files / 340 tests and a zero-warning submission audit. Still not done: any onchain write, ClawPump funded launch, Meteora signatures, a production database, and videos. The per-task tables below are the original plan and are kept for traceability; where they say "deferred" for fresh proposals, the update above supersedes them.
