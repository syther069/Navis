...
import { executionIntents } from "@workspace/navis-core/lib/db/schema";
...
export async function POST(request: NextRequest) {
  ...
  if (!executionSimulationAvailable()) {
    return NextResponse.json(
      {
        error:
          "Meteora pool simulation requires devnet or mainnet execution mode with SOLANA_RPC_URL configured.",
      },
      { status: 409 },
    );
  }
  ...
  if (!session) {
    return NextResponse.json(
      { error: "Authenticate a connected wallet before simulating a Meteora pool." },
      { status: 401 },
    );
  }
    ...
    return NextResponse.json(
      {
        error: meteoraPublicErrorMessage(
          error,
          "Meteora pool transaction could not be simulated.",
        ),
      },
