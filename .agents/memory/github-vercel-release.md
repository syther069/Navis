...
description: Why pushes to the NAVIS GitHub main branch fail and how the repo must stay deployable on Vercel.
---
...
- Replit's GitHub integration cannot write to the NAVIS repo (403 "Resource not accessible by integration"). Pushes use the user's fine-grained token in secret GITHUB_PUSH_TOKEN via GIT_ASKPASS; it needs Contents and Workflows read/write (Workflows because `.github/workflows` files moved).
