...
describe("stock pair ticker filter in table mode", () => {
  afterEach(cleanup);

  it("renders rows under the given head and hides non-matching rows", () => {
    const { container } = render(
      ...
      >
        {PAIRS.map((pair) => (
          <tr key={pair.mint} data-pair-mint={pair.mint}>
            <td>{pair.name}</td>
          </tr>
        ))}
      </StockPairFilter>,
    ...
    expect(screen.getByText("Tokenized stocks (1)")).toBeTruthy();
    expect(slot(container, "mint-aapl").hidden).toBe(false);
    expect(slot(container, "mint-nvda").hidden).toBe(true);
  });
});
